...
###DO#######
file.names <- list.files(path="HOBO Excels/14/DO", pattern=".csv", full.names=TRUE)
...
file.names <- list.files(path="HOBO Excels/14/SpC", pattern=".csv", full.names=TRUE)
...
####pH#####
file.names <- list.files(path="HOBO Excels/14/pH", pattern=".xlsx", full.names=TRUE)
...
####Lily Box#######
file.names <- list.files(path="Lily Box/csv/14", pattern=".csv", full.names=TRUE)
...
file.names <- list.files(path="Lily Box/dat/14", pattern=".dat", full.names=TRUE)
...
ggplot(LB_14FDOM_dat, aes(x=Date))+
  geom_line(aes(y=FDOM), size=0.8)
...
ggplot(LB_14CO2_dat, aes(x=Date))+
  geom_line(aes(y=CO2), size=0.8)
...
h14 <- read_excel("//ad.ufl.edu/ifas/SFRC/Groups/Hydrology/Bradford_Forest_Project/Streams/Stream_H20_level/Calculated_Stage/Stream #3.xlsx", 
                 skip = 1)
...
S14$Site<-"14"
write_xlsx(S14, "//ad.ufl.edu/ifas/SFRC/Groups/Hydrology/Bradford_Forest_Project/Masterfiles_latest/Stream Chemistry/14.xlsx")
