...
theme_label<-theme()+theme(axis.text.x = element_text(size = 19, angle=0),
                           axis.text.y = element_text(size = 19, angle=0),
                           axis.title.y =element_text(size = 19, color = "black"),
...
FDOM_all$Site <- factor(FDOM_all$Site, levels=c("3","5","5a",'6',"6a","7",
                                        "9", "13","14","15")) #prefer this organization

FDOM<-ggplot(FDOM_all,aes(x=Site,y=FDOM))+
  geom_boxplot(outlier.color="black")+
  ggtitle("FDOM")+
  ylab("ppb QS")+theme_label
...
ggsave(filename="05_Figures/boxplots.jpeg",
       plot = box,
       width =19,
       height = 14.5,
       units = "in")
