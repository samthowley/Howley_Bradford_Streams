...
write_csv(DG1, '01_Raw_data/DG/6a/12192023_6a.csv')
...
depth<-depth[,-c(1)]
depth <- depth %>% group_by(day,ID)%>% mutate(depth_mean= mean(depth, na.rm = T))

DG<-left_join(DG, depth, by=c('ID','day'))
x<-c("day","ID","Q","u_mean" ,"depth_mean","m_0","m_1")
DG<-DG[,x]
DG <- DG[!duplicated(DG[c('day','ID')]),]

split<-DG %>% split(DG$ID)
...
file.names <- list.files(path="01_Raw_data/DG/6a", pattern=".csv", full.names=TRUE)
DG_all<-data.frame()
for(i in file.names){
  DG<-read_csv(i)
