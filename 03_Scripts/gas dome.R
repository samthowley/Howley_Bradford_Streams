...
Q<-read_csv('02_Clean_data/depth.csv')
Q<-Q %>% mutate(Date= as.Date(Date)) %>% group_by(Date,ID) %>% mutate(h_avg=mean(depth, na.rm=T))
Q<-Q[,c(1,4,5)]
...
colnames(gas)[1] <- "Date"
colnames(gas)[2] <- "CO2"
gas<-gas %>%filter(Date>'2024-02-10') %>%filter(CO2<100)
