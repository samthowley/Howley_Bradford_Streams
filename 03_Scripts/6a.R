...
DO_6a_all$DO[DO_6a_all$DO<0] <- 0.01
DO_6a_all<- filter(DO_6a_all, DO<6)
S6a<-left_join(samplingperiod, DO_6a_all, by='Date')

ggplot(DO_6a_all, aes(x=Date))+
...
SpC_6a_all<- filter(SpC_6a_all, SpC>50 &SpC<300)
S6a<-left_join(S6a, SpC_6a_all, by='Date')
ggplot(SpC_6a_all, aes(x=Date))+
...
for(fil in file.names){
  ...
  colnames(pH6a)[2] <- "pH"
  #pH6a$Date <- mdy_hms(pH6a$Date)
  pH_6a_all <- rbind(pH_6a_all, pH6a)
}


pH_6a_all<- filter(pH_6a_all, pH<6.9)
S6a<-left_join(S6a, pH_6a_all, by='Date')
ggplot(pH_6a_all, aes(x=Date))+
...
for(fil in file.names){
  ...
  LB6a<-filter(LB6a,FDOM>1)
  LB_6aFDOM_csv <- rbind(LB_6aFDOM_csv, LB6a) }

ggplot(LB_6aFDOM_csv, aes(x=Date))+
...
ggplot(LB_6aFDOM_dat, aes(x=Date))+
  geom_line(aes(y=FDOM), size=0.8)
...
for(fil in file.names){
  ...
  LB6a<-LB6a[,c(1,5)]
  colnames(LB6a)[3] <- "CO2"
  LB6a<-filter(LB6a, CO2> 500)
  LB_6aCO2_csv <- rbind(LB_6aCO2_csv, LB6a) }

ggplot(LB_6aCO2_csv, aes(x=Date))+
