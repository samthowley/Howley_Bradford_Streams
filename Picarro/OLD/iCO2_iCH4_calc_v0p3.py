...
def MergeResults():
   ...
   'HP_Delta_iCH4_Raw', '12CO2','13CO2','Delta_Raw_iCO2', 'H2O', 'OutletValve', 'ChemDetect'])
...
def askopenresultsfilename():
   ...
   if sampletimefilename:
      ...
      resultswriter.writerow(['SampleName', 'Rundate','Runtime', 'Position', '12CH4mean', 
      '12CH4slope', '12CH4intercept','13CH4mean','13CH4slope','13CH4intercept', 'd13C_CH4mean',
      ...
      for row in sampletimes['SampleName']:
         xsec=[]; y12CH4=[]; y13CH4=[]; yd13CCH4=[]; y12CO2=[]; y13CO2=[]; yd13CCO2=[]; yH2O=[]; yChemDetect=[]; yOutletValve=[];
         xsecs=[]; y12CH4s=[]; y13CH4s=[]; yd13CCH4s=[]; y12CO2s=[]; y13CO2s=[]; yd13CCO2s=[]; yH2Os=[]; yChemDetects=[]; yOutletValves=[];
         ...
         if len(y12CH4s)>2:
            ...
            _13CO2mean, _13CO2slope,_13CO2intercept,d13CCO2mean,d13CCO2slope,d13CCO2intercept,H2Omean,OutletValvemean,ChemDetectmean])
            ...
            line4.grid()
            line4.set_ylabel('OutletValve (0=open, 65536=closed)', color='b')
...
root = Tk()
root.title('iCH4 iCO2 results calculator 20190210')

#__________________________________LOGO&TITLE________________________________________

bigtitle = Label(root, anchor=W, font=('times', 20, 'bold'), fg='white',bg='blue', text="iCH4 & iCO2 calculator ")
...
UTCoffsettitle = Label(root, anchor=W, text="Offset local time UTC (Google UTC now):")
UTCoffsettitle.grid(row=13,column=0, columnspan=1, sticky=[E])
UTCoffsetentry= Entry(root,width=4)
UTCoffsetentry.insert(0,"-5")
