...
# this is the list of Picarro variables to report on (with slope and intercept if chosen)
varsToSummarize=['HP_12CH4','HP_13CH4','HP_Delta_iCH4_Raw','HR_12CH4','HR_13CH4','HR_Delta_iCH4_Raw','12CO2','13CO2','Delta_Raw_iCO2']
# this is the list of Picarro variables to report on (without slope and intercept if chosen)
        ...
        if len(sampleDict['xsec'])>2:
            ...
            if doSlopeIntercept.get()>0: 
                resultswriter.writerow([sampletimes['SampleName'][c1],rundate,runtime, sampletimes['Position'][c1],
                   (len(varsToSummarize)*3+len(varsToSummarizeStatic))*('na',)])
            else:
                resultswriter.writerow([sampletimes['SampleName'][c1],rundate,runtime, sampletimes['Position'][c1],
                   (len(varsToSummarize)*1+len(varsToSummarizeStatic))*('na',)])
...
root = Tk()
root.title('iCH4 iCO2 results calculator 20191017')
...
# uncomment next line if you always want slope and intercepts saved
#doSlopeInterceptApply.select()
doSlopeInterceptApply.grid(row=14,column=0, columnspan=1, sticky=W)

doSaveStdev=IntVar()
doSaveStdevApply = Checkbutton(root, text="Record Stdev not implemeted)", variable=doSaveStdev)
# uncommentnext line if you always want Stdev saved
